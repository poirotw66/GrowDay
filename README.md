...
- **核心功能**：單一與多習慣管理、電子寵物成長與退休、成就系統、世界地圖與裝飾、統計圖表、目標設定與每日日曆打卡皆已可正常使用。
- **資料儲存**：支援本機儲存，並可選配 Firebase 雲端同步與 Google 登入；若未設定 Firebase 環境變數，應用仍可在純本機模式下運作。
- **平台支援**：已優化桌面與手機版版面配置，並提供 PWA 安裝與離線使用的基本體驗。
- **測試與品質**：已設定 Vitest + Testing Library 測試環境與 ESLint/Prettier 規則，測試案例仍在逐步補齊中。
- **未來規劃**：更多遊戲化要素、統計視覺化與世界互動細節可參考 `docs/FEATURE_IDEAS.md`，歡迎依照文件中的建議優先順序貢獻功能。
